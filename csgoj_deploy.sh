...
interactive_configure_web() {
    ...
    # 设置派生配置：如果用户没有明确提供 BELONG_TO，则设置为 OJ_NAME
    # 注意：PROVIDED_BELONG_TO 在 parse_args 中设置，如果交互式配置在 parse_args 之后调用，
    # 需要检查 BELONG_TO 是否等于默认值或空值，如果是则设置为 OJ_NAME
    if [ "$PROVIDED_BELONG_TO" != "true" ]; then
        # 用户没有明确提供 BELONG_TO，设置为 OJ_NAME
        BELONG_TO="$OJ_NAME"
    fi
