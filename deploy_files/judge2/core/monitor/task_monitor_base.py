                ...
                try:
                    ...
                    if os.path.exists(procs_file):
                        ...
                        if pids:
                            ...
                            # 检查是否还有进程
                            with open(procs_file, 'r') as f:
                                remaining_pids = [line.strip() for line in f.readlines() if line.strip()]
                    ...
                    if retry == max_retries - 1:
                        self.logger.warning(f"清理cgroup进程列表失败（重试{max_retries}次）: {e}")
                    else:
                        import time
                        time.sleep(0.1)
                        continue
            ...
            for retry in range(max_retries):
                try:
                    ...
                    if retry < max_retries - 1:
                        ...
                        # 最后一次尝试失败，记录警告
                        self.logger.warning(f"删除cgroup目录失败（重试{max_retries}次）: {self.cgroup_path}, 错误: {e}")
                        # 检查目录是否还存在
                            ...
                            try:
                                ...
                                if os.path.exists(procs_file):
                                    with open(procs_file, 'r') as f:
                                        remaining_pids = [line.strip() for line in f.readlines() if line.strip()]
                                    if remaining_pids:
                                        self.logger.warning(f"cgroup中仍有{len(remaining_pids)}个进程未清理: {remaining_pids}")
                            except Exception:
                                pass
        except Exception as e:
            self.logger.warning(f"清理cgroup时发生异常: {e}")
