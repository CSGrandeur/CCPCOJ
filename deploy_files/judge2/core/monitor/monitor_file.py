...
class MonitorFile:
    ...
    def _mount_system_dependencies(self):
        ...
        注意：此方法只挂载所有语言都需要的通用系统目录（如动态库、设备文件等）。
        语言特定的目录（如 /usr/bin、/bin 等）由各语言处理器的 get_compile_dependency_dirs()
        和 get_runtime_dependency_dirs() 方法定义，在 _mount_language_dependencies() 中挂载。
        """
        ...
        if self.task_type in [sc.TASK_TYPE_PLAYER_COMPILE, sc.TASK_TYPE_TPJ_COMPILE, 
            ...
            # 运行任务：只挂载所有语言都需要的通用系统目录
            # 动态库目录（所有语言的可执行文件都可能依赖）
            # 设备文件（所有语言都需要）
            # 注意：语言特定的目录（如 /usr/bin、/bin 等）由语言处理器定义
    ...
    def _mount_language_dependencies(self):
        ...
        for dep_dir in deps:
            # 检查是否包含通配符
            if '*' in dep_dir or '?' in dep_dir:
                # 使用 glob 展开通配符
                matched = glob.glob(dep_dir)
                if matched:
                    actual_deps.extend(matched)
                elif is_debug_enabled():
                    self.logger.debug(f"依赖目录 glob 模式未匹配: {dep_dir}")
            else:
                # 直接使用路径
                actual_deps.append(dep_dir)
        
        # 挂载语言依赖目录
        # 注意：_mount_single_point 内部会检查是否已挂载，避免重复挂载
        for dep_dir in actual_deps:
