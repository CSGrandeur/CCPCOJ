...
validate_and_calculate_resources() {
    ...
    if [ $target_pod_count -eq 1 ] && [ $total_cpus -lt 8 ]; then
        ...
        # 考虑已存在的 pod 数量
        local existing_pods=$(docker ps -a --filter "name=judge-$OJ_NAME" --format "{{.Names}}" 2>/dev/null | wc -l)
        local fixed_cpus_per_pod=4
        local fixed_offset=2
        # 已存在的 pod 占用的 CPU = existing_pods * fixed_cpus_per_pod
        local used_cpus=$((existing_pods * fixed_cpus_per_pod))
        # 可用 CPU = 总CPU - 固定偏移 - 已占用的CPU
        local available_after_offset=$((total_cpus - fixed_offset - used_cpus))
...
start_single_pod() {
    ...
    if [ $cpu_offset -gt 0 ]; then
        ...
        # 边界检查：确保CPU分配不超出系统总CPU数
        if [ $cpu_end -ge $total_cpus ]; then
            echo "  ❌ 错误: Pod $pod_index 的CPU分配超出系统范围"
            echo "     请求CPU: $cpu_start-$cpu_end (共 $((cpu_end - cpu_start + 1)) 个)"
            echo "     系统CPU总数: $total_cpus (0-$((total_cpus - 1)))"
            echo "     已分配的CPU: $cpu_offset-$((cpu_offset + cpus * pod_index - 1))"
            echo ""
            echo "💡 建议:"
            echo "   1. 减少 pod 数量"
        ...
        echo "  📌 CPU 绑定: 逻辑核心 $cpu_start-$cpu_end (共 $cpus 个核心)"
    else
