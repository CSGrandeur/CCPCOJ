...
function OneSample(i, sample_in_item, sample_out_item, hlevel=4, is_input_dom=false) {
    ...
    const copy_btn = `<button type="button" class="btn btn-outline-secondary btn-sm sample_copy" title="复制 (Copy)">
                ...
                <div class="row g-0">
                    <div class="col-6 border-end">
                        ...
                        <div class="sample-content p-3">
                            <${sample_txt_area_type} class="sampledata sample_input_area ${clss}" ${ith_info} stype="input" ${is_input_dom ? 'rows="4"' : ''}>${sample_in_item}</${sample_txt_area_type}>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="sample-section-header bg-light border-bottom px-3 py-2 d-flex justify-content-between align-items-center">
                        ...
                        <div class="sample-content p-3">
                            <${sample_txt_area_type} class="sampledata sample_output_area ${clss}" ${ith_info} stype="output" ${is_input_dom ? 'rows="4"' : ''}>${sample_out_item}</${sample_txt_area_type}>
                        </div>
                    </div>
                </div>
            </div>
...
document.addEventListener('click', (e) => {
    if(e.target.classList.contains("sample_copy") || e.target.closest(".sample_copy")) {
        ...
        let target_sample = btn.closest('.sample-item').querySelector('.sampledata');
