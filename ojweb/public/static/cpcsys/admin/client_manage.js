        ...
        success: function (data) {
            ...
            if (data && Array.isArray(data)) {
                data.forEach((row, index) => {
                    if (!row.row_index) {
                        row.row_index = `server_${row.client_id || index}`;
                    }
                });
            }
...
function ClientParseTextData() {
    ...
    ClientShowPreview(clientData, errors);
}
...
function ClientValidateData(clientInfo, rowNumber) {
    ...
    if (!clientInfo.team_id_bind || clientInfo.team_id_bind.trim() === "") {
        errors.push(`Row ${rowNumber}: team_id_bind is required`);
    }
    
    if (!clientInfo.ip_bind || clientInfo.ip_bind.trim() === "") {
        errors.push(`Row ${rowNumber}: ip_bind is required`);
    }
    ...
    // 如果所有 SSH 字段都为空，则通过
    if (sshUser === "" && sshPass === "" && sshRsa === "" && sshPort === "") {
        // 通过验证
    } else {
        // 如果提供了 SSH 配置，则 user 和 port 必填
        if (sshUser === "" || sshPort === "") {
            errors.push(`Row ${rowNumber}: SSH user and port are required when SSH config is provided`);
        }
        
        // pass 和 rsa 至少需要有一个
        if (sshPass === "" && sshRsa === "") {
            errors.push(`Row ${rowNumber}: SSH password or RSA key is required`);
        }
    }
...
// 显示预览
function ClientShowPreview(clientData, errors) {
    ...
    // 更新工具栏
    $("#toolbar_title").html(
        '<span class="cn-text"><i class="bi bi-eye me-2"></i>数据预览</span><span class="en-text">Data Preview</span>'
    );
    $("#toolbar_subtitle")
    ...
    if (errorRows.length > 0) {
        $("#error_count").text(errorRows.length);
        $("#error_summary").show();
    } else {
        $("#error_summary").hide();
    }
    ...
    if (errorCount > 0) {
        window.alerty.warning(
            `解析完成：共 ${totalRows} 行，其中 ${errorCount} 行有错误 / Parsed: ${totalRows} rows, ${errorCount} errors`
        );
    } else {
        window.alerty.success(
            `解析完成：共 ${totalRows} 行数据 / Parsed: ${totalRows} rows successfully`
        );
    }
}
...
async function ClientExportToExcel(clientData, filename) {
    try {
        ...
        // 设置列定义
        const clientHeaders = [
            { header: "ID", header_en: "ID", key: "client_id", width: 10 },
            { header: "队伍号", header_en: "Team ID", key: "team_id_bind", width: 15 },
            { header: "IP地址", header_en: "IP Address", key: "ip_bind", width: 15 },
            { header: "SSH用户", header_en: "SSH User", key: "ssh_user", width: 15 },
            { header: "SSH密码", header_en: "SSH Password", key: "ssh_pass", width: 15 },
...
function ClientDeleteFromPreview(row) {
    ...
    if (index >= 0) {
        ...
        // 重新显示预览
        const errors = [];
        client_PreviewData.forEach(client => {
            if (client.validation_errors && client.validation_errors.length > 0) {
                errors.push(...client.validation_errors);
            }
        });
        ClientShowPreview(client_PreviewData, errors);
        window.alerty.success("已从预览中删除 / Removed from preview");
    }
}
