...
function FormatterOutrankTitle(value, row, index, field) {
    return `<a class="text-decoration-none text-primary edit-outrank-link" href="/outrank/rank?outrank_uuid=${row.outrank_uuid}" data-id="${row.outrank_id}" title="${value}">${value}</a>`;
}
...
$(document).ready(function() {
    ...
    // 编辑按钮事件
    $(document).on('click', '.edit-outrank-btn, .edit-outrank-link', function() {
    ...
    function bindCellEvents() {
        ...
        table.on('click-cell.bs.table', async function(e, field, td, row) {
            ...
            } else if (field === 'title' && row.is_admin) {
                // 单击标题列（管理员）：打开编辑modal
                openOutrankEditModal(row.outrank_id);
            }
        });
